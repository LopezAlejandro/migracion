...
$identificador_nc = $parametros['GENERAL']['identificador_nc'];
...
function registro_marcXML($registro, $config)
{ // registro{mfn, campos=array ( campo{id, valor} ) }
	$r = new stdClass();
	$r->mfn = $registro->mfn;
	$r->campos = array();
	...
	$control["001"] = $config['marc040a'];

	$datos["040"][$mfn] = new stdClass();
	$datos['040'][$mfn]->i1 = ' ';
	$datos['040'][$mfn]->i2 = ' ';
	$datos["040"][$mfn]->a = 'AR-BaUAD';
	$datos["040"][$mfn]->c = 'AR-BaUAD';
	$datos["040"][$mfn]->b = 'spa';
	...
	$tipo_documento = isset($r->campos['v007']) ? $r->campos['v007'][0] : "REVISTA"; // TIPO DE DOCUMENTO

	$datos['998'][$i] = new stdClass();
	$datos['998'][$i]->i1 = ' ';
	$datos['998'][$i]->i2 = '';
	$datos['998'][$i]->a = "AAQ";
	$datos['998'][$i]->b = "BIBUN76";

	$datos['942'][$i] = new stdClass();
	$datos['942'][$i]->i1 = ' ';
	$datos['942'][$i]->i2 = ''; //item type
	...
	$soporte = isset($r->campos['v008']) ? $r->campos['v008'][0] : "PAPEL"; // 008: NIVEL DE DESCRIPCIÓN BIBLIOGRÁFICA
//	$datos['338'][$i] = new stdClass();
//	$datos['338'][$i]->i1 = ' ';
//	$datos['338'][$i]->i2 = ' ';
//	$datos['338'][$i]->a = strtolower($soporte);

	$control['001'] = isset($r->campos['v001']) ? $config['marc040a'] . $r->campos['v001'][0] : "No posee"; // Nro de control
	$control['003'] = $config['identificador_nc'];

	#isset($r->campos['v076'])?$r->campos['v076'][0]:"";// Identificador del número de control
	...
	#	$datos['046'][$i] = new stdClass();
	#	$datos['046'][$i]->i1 = ' ';$datos['046'][$i]->i2 = ' ';
		...
		switch ($campo->id) {
			...
			case '001': //NUMERO DE ACCESO
				if (!isset($datos['952'][$registro->mfn])) {
					$datos['952'][$registro->mfn] = new stdClass();
					$datos['952'][$registro->mfn]->i1 = ' ';
					$datos['952'][$registro->mfn]->i2 = ' ';
				}
				$datos['952'][$registro->mfn]->p = 'AAQA' . $campo->valor;
				$datos['952'][$registro->mfn]->a = 'AR-BAuAD';
				$datos['952'][$registro->mfn]->b = 'AR-BAuAD';
				$datos['952'][$registro->mfn]->y = 'ARTI';
				break;
			...
			case '015': //015 ISSN - Longitud 9 - ej 9501-5187
				if (!isset($datos['022'][$registro->mfn])) {
					$datos['022'][$registro->mfn] = new stdClass();
					$datos['022'][$registro->mfn]->i1 = ' ';
					$datos['022'][$registro->mfn]->i2 = ' ';
				}
				$datos['022'][$registro->mfn]->c = $campo->valor;
				break;
...
//				$datos['245'][$i]->h = isset($soporte) ? '['.strtolower($soporte).']' : '';
			...
			case '036': //036 TITULO (PUBLICACIÓN EN SERIE)
				if (!isset($datos['773'][$registro->mfn])){
				$datos['773'][$i] = new stdClass();
				$datos['773'][$i]->i1 = '0';
				$datos['773'][$i]->i2 = ' ';
				}
				//Obtiene el desde el inicio hasta la coma
				$tit = trim(strstr(subcampo($campo->valor, 't'), ',', true));

				if (strlen($tit) == 0) {
					//obtiene desde el inicio hasta el [
					$tit = trim(strstr(subcampo($campo->valor, 't'), '[', true));
					//obtiene desde [ inclusive hasta el final
					$resto = trim(substr(strstr(subcampo($campo->valor, 't'), '['), 0));
				} else {
					$resto = trim(substr(strstr(subcampo($campo->valor, 't'), ','), 1));
				}

				$datos['773'][$i]->t = $tit;
				$datos['773'][$i]->g = $resto;
				if (subcampo($campo->valor, 's')) {
					$datos['773'][$i]->a = "(" . subcampo($campo->valor, 's') . ")";
				}
				if ($entrada_principal == 'TI') {
					$datos['130'][$i] = new stdClass();
					$datos['130'][$i]->i1 = ' ';
			...
			case '028':
				...
				$datos['245'][$i]->i1 = '1';
				$datos['245'][$i]->i2 = '0';
				$campoAutorPersonal = subcampo($campo->valor, 'b') . " " . subcampo($campo->valor, 'a');
				...
				if ($entrada_principal == 'PE' && !isset($datos['100'])) {
					$datos['100'][$mfn] = new stdClass();
					$datos['100'][$mfn]->i1 = ' ';
					$datos['100'][$mfn]->i2 = ' ';
					$datos['100'][$mfn]->a = subcampo($campo->valor, 'a') . ", " . subcampo($campo->valor, 'b');
				}
				$datos['245'][$i]->c = $campoAutorPersonal;

				// Agregar al campo 700 si no es el autor principal
				static $counter700 = 0;
				if ($entrada_principal != 'PE' || (isset($datos['100'][$mfn]) && $datos['100'][$mfn]->a != subcampo($campo->valor, 'a') . ", " . subcampo($campo->valor, 'b'))) {
					$datos['700'][$counter700] = new stdClass();
					$datos['700'][$counter700]->i1 = '1';
					$datos['700'][$counter700]->i2 = '2';
					$datos['700'][$counter700]->a = subcampo($campo->valor, 'a') . ", " . subcampo($campo->valor, 'b');
					$counter700++;
				}
				break;
			case '033': //033 AUTOR PERSONAL (NIVEL COLECCIÓN)
				if ($entrada_principal == 'PE') {
					$datos['100'][$i] = new stdClass();
					$datos['100'][$i]->i1 = ' ';
					$datos['100'][$i]->i2 = ' ';
					$datos['100'][$i]->a = subcampo($campo->valor, 'a') . ", " . subcampo($campo->valor, 'b');
				}
				$datos['508'][$i] = new stdClass();
				$datos['508'][$i]->i1 = ' ';
				$datos['508'][$i]->i2 = ' ';
			...
			case '034': //RESPONSABLE INSTITUCIONAL (NIVEL COLECCIÓN)
				...
				if (subcampo($campo->valor, 'e')) {
					$datos['508'][$i]->a .= subcampo($campo->valor, 'e');
				}
				if ($siglas) {
					$datos['508'][$i]->a .= $siglas;
				}
				if ($pais) {
					$datos['508'][$i]->a .= $pais;
				}
			...
			case '040': //040 NOMBRE DE LA REUNIÓN - ^n Nombre de la reunión.^x Número, se coloca en números arábigos y sinabreviatura del ordinal.^o
				if (!isset($datos['111'][$registro->mfn])) {
					$datos['111'][$registro->mfn] = new stdClass();
					$datos['111'][$registro->mfn]->i1 = ' ';
					$datos['111'][$registro->mfn]->i2 = ' ';
				}
			...
			case '043': //043 RESPONSABLE DE LA REUNIÓN
				if (!isset($datos['647'][$registro->mfn])) {
					$datos['647'][$registro->mfn] = new stdClass();
					$datos['647'][$registro->mfn]->i1 = ' ';
					$datos['647'][$registro->mfn]->i2 = ' ';
			...
			case '045': //045 FECHA DE PUBLICACIÓN (seriada)
				if (!isset($datos['260'][$registro->mfn])) {
					$datos['260'][$registro->mfn] = new stdClass();
					$datos['260'][$registro->mfn]->i1 = ' ';
					$datos['260'][$registro->mfn]->i2 = ' ';
				}
			...
			case '048': //048 PAÍS DE EDICIÓN - código de país
				if (!isset($datos['044'][$registro->mfn])) {
					$datos['044'][$registro->mfn] = new stdClass();
					$datos['044'][$registro->mfn]->i1 = ' ';
					$datos['044'][$registro->mfn]->i2 = ' ';
				}
				$datos['044'][$registro->mfn]->b = tabla_pais($campo->valor);
				break;
			case '050': //050 IDIOMA DEL DOCUMENTO
				if (!isset($datos['041'][$registro->mfn])) {
					$datos['041'][$registro->mfn] = new stdClass();
					$datos['041'][$registro->mfn]->i1 = ' ';
					$datos['041'][$registro->mfn]->i2 = ' ';
				}
				$datos['041'][$registro->mfn]->b = tabla_idiomas($campo->valor);
				break;
			case '053': //052 DESCRIPCIÓN FÍSICA - ^e Extensión. Páginas, volúmenes o cantidad de piezas. ^i Material ilustrativo. Ilustraciones, 
				if (!isset($datos['773'][$registro->mfn])) {
					$datos['773'][$registro->mfn] = new stdClass();
					$datos['773'][$registro->mfn]->i1 = '0';
					$datos['773'][$registro->mfn]->i2 = ' ';
				}
				$datos['773'][$registro->mfn]->g = ($campo->valor);
//				$datos['300'][$registro->mfn]->b = subcampo($campo->valor, 'i') . subcampo($campo->valor, 'c');
			...
			case "060": //060 CLASIFICACIÓN TEMÁTICA - segun CDU
				if (!isset($datos['080'][$registro->mfn])) {
					$datos['080'][$registro->mfn] = new stdClass();
					$datos['080'][$registro->mfn]->i1 = ' ';
					$datos['080'][$registro->mfn]->i2 = ' ';
				}
			...
			case "206": // descriptores temporales
				$datos['653'][$i] = new stdClass();
				$datos['653'][$i]->i1 = ' ';
				$datos['653'][$i]->i2 = '4 ';
				$datos['653'][$i]->a = $campo->valor;
				break;
			case "069": //069 RESUMEN
				$datos['520'][$i] = new stdClass();
				$datos['520'][$i]->i1 = ' ';
				$datos['520'][$i]->i2 = ' ';
			...
			case "079": //079 REGISTROS VINCULADOS-formato de salida: "^a".$inventario."^d".$disponibilidad."^p".$precio."^v".$volumen_ejemplar."^m".$adquisición;
				...
				break;
			case "205":
				$datos['990'][$i] = new stdClass();
				$datos['990'][$i]->i1 = ' ';
				$datos['990'][$i]->i2 = ' ';
				if (subcampo($campo->valor, 'a')) {
					$datos['990'][$i]->p = subcampo($campo->valor, 'a');
					$datos['990'][$i]->i = subcampo($campo->valor, 'a');
					$datos['990'][$i]->f = subcampo($campo->valor, 'd');
					$datos['990'][$i]->g = subcampo($campo->valor, 'p');
					$datos['990'][$i]->t = subcampo($campo->valor, 'v');
					$datos['990'][$i]->e = subcampo($campo->valor, 'm');
					$datos['990'][$i]->a = subcampo($campo->valor, 'l');
					$datos['990'][$i]->c = subcampo($campo->valor, 'e');
				} else {
					$datos['990'][$i]->a = $campo->valor;
					$datos['990'][$i]->b = 'TD';
				}
				break;

			case "856":
				$datos['856'][$i] = new stdClass();
				$datos['856'][$i]->i1 = '4';
				$datos['856'][$i]->i2 = '#';
				$datos['856'][$i]->u = subcampo($campo->valor, 'u');
				break;
...
function leader($tamanio, $pos_base, $tipo = 'b')
{
	$cabecera = str_pad($tamanio, 5, "0", STR_PAD_LEFT);
	$cabecera .= "n"; //05 - n: reg nuevo
	...
	$cabecera .= $tipo; //07 - m: monográfico - s: seriada - b: componente seriada
	...
	$cabecera .= "7"; //17 -  : nivel de codificacion, no especifica codificacion
...
function campo_control_008($fecha, $publicacion = "xx#", $ilustración = "#|#p", $tipo_item = "#", $idioma = "es")
{
...
function tabla_idiomas($cod_iso)
{
	switch (strtolower($cod_iso)) {
		...
		case "ru":
			$idioma = 'rum';
			break;  //Ruso  ru -> romanian
		case "su":
			$idioma = 'fin';
			break;  //finlandes -> finnish
		case "ni":
			$idioma = 'dut';
			break; //holandés
		case "ar":
		...
		case "cs":
			$idioma = "cze";
			break; //checo (el código bibun para checo es igual al de español)
		default:
			$idioma = 'BIBUN:' . $cod_iso;
			$idioma = "iso: " . $cod_iso;
	}
...
function tabla_pais($cod_iso)
{
	...
	switch (strtolower($cod_iso)) {
		...
		case "cs":
			$pais = 'cs';
			break;  //CHECOSLOVAQUIA - descontinuado
		case "cu":
			$pais = 'cu';
			break;  //CUBA
		case "ch":
